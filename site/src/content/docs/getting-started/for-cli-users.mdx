...
For image-generation workflows, pull assets into the unified model store and
start ComfyUI:

```bash
swm setup install comfyui runpod:YOUR_POD_ID
swm models pull runpod:YOUR_POD_ID civitai:101055 --as checkpoint
swm models pull runpod:YOUR_POD_ID https://example.com/style.safetensors --as lora
swm setup start comfyui runpod:YOUR_POD_ID
```

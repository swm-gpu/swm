...
`swm pod create` doesn't just pull your workspace — it leaves the pod with a running auto-sync daemon. The daemon wakes every 60 seconds, reconciles the inotify watcher log with a bounded filesystem scan, and pushes new/changed files (and removes deleted ones) without you running anything. Inspect or change it with:
...
**Reconciliation scan.** inotify can miss files created inside brand-new directories during very fast bursts such as `pip install` or `git clone`. Auto-sync therefore combines watcher events with a high-watermark `find -newer` scan before advancing the push stamp. Files written while a sync cycle is running remain eligible for the next cycle.
